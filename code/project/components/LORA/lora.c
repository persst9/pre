...
void uart_init(void)
{
     ...
     * - 0：驱动安装标志（无特殊标志）
     */
    uart_driver_install(UART_NUM_1, receive_buffer_size , send_buffer_size , 
                        20, NULL, 0);
}
...
{
    uart_write_bytes(UART_NUM_1, (const char *)data, len); // 发送数据
}

void lora_receive(uint8_t *data, uint16_t len)
{
    uart_read_bytes(UART_NUM_1, data, len, 100); // 接收数据
}
